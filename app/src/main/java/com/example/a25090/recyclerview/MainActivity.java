...
private RecyclerView recyclerView;
    ...
    private SwipeRefreshLayout mSwipeRefrishLayout;//上拉刷新事件
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSwipeRefrishLayout= (SwipeRefreshLayout) findViewById(R.id.swipe_refresh_layout);
        initData();
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        mMenuAdapter = new MyAdapter(this,datalist);
        recyclerView.addItemDecoration(new MenuItemDecoration());//关联分割线文件
        recyclerView.setAdapter(mMenuAdapter);

        mSwipeRefrishLayout.setOnRefreshListener(this);
        recyclerView.addOnScrollListener(new MenuOnScrollListener() {
            @Override
            public void loadMore() {
                Map<String,Object> map=new HashMap<>();
                map.put("menu_thumb",R.drawable.gongbaojiding);
    ...
    public void onRefresh() {
        recyclerView.removeAllViews();//下拉刷新前先把所有的列表项移除掉
        datalist.clear();//清除数据
        for (int i=0;i<=10;i++){
            Map<String, Object> map = new HashMap<>();
